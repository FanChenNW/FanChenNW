      ...
      double precision t2,stv
      integer, parameter :: nds =2784             !C5
C     integer, parameter :: nds =21888            !C3
C     integer, parameter :: nds =9288             !C2
C     integer, parameter :: nds =45880            !C1
      common /GID/ grainID(nds),xgr(nds),ygr(nds),zgr(nds)
...
CCLOP=0 indicates that the subroutine is being called at the start of the analysis.
CC
CCLOP=1 indicates that the subroutine is being called at the start of the current 
Canalysis increment. The subroutine can be called multiple times at the beginning 
Cof an analysis increment if the increment fails to converge and a smaller time increment is required.
CC
CCLOP=2 indicates that the subroutine is being called at the end of the current analysis 
Cincrement. When LOP=2, all information that you need to restart the analysis should be written to external files.
CC
CCLOP=3 indicates that the subroutine is being called at the end of the analysis.
CC
CCLOP=4 indicates that the subroutine is being called at the beginning of a restart analysis. When LOP=4, 
Call necessary external files should be opened and properly positioned and all information required for 
Cthe restart should be read from the external files.

       if (LOP.eq.0) then
       call MutexInit(1)! initialize Mutex #1
       call MutexInit(2)! initialize Mutex #2
       call MutexInit(3)! initialize Mutex #3
       call MutexInit(4)! initialize Mutex #4

       endif
       
       if (LOP.eq.1) then
       print*,'KINC,LOP',KINC,lop
       if (KINC.eq.1) then
      ...
      call MutexLock( 4 )      ! lock Mutex #4
         grainID(iii)=1
      call MutexUnlock( 4 )      ! lock Mutex #4
      enddo
      ...
      call getGrainID (stv,xgr(iii),ygr(iii),zgr(iii),t2)
      ...
      double precision x, y, z, t1, t2
      integer, parameter :: t_stp_num =8
      integer, parameter :: x_num = 51
      integer, parameter :: y_num = 121
      integer, parameter :: z_num = 34
      Integer IncN(t_stp_num), u
      double precision IncT(t_stp_num)
      Integer NInc
      Integer IncN1, IncN2,ttta
      double precision IncT1, IncT2, head,NNT1,nnt11
      double precision NNT2
      double precision, ALLOCATABLE, DIMENSION(:,:,:) :: NT1
      double precision, ALLOCATABLE, DIMENSION(:,:,:) :: NT2
      double precision, ALLOCATABLE, DIMENSION(:,:,:) :: NTl1
      double precision, ALLOCATABLE, DIMENSION(:,:,:) :: NTl2
      double precision, ALLOCATABLE, DIMENSION(:,:,:) :: NTh1
      double precision, ALLOCATABLE, DIMENSION(:,:,:) :: NTh2
      double precision, ALLOCATABLE, DIMENSION(:,:,:) :: NTAVG
      double precision, save :: xc1(x_num), yc1(y_num), zc1(z_num)
      double precision, save :: xc2(x_num), yc2(y_num), zc2(z_num)
      double precision, save :: xc(x_num), yc(y_num), zc(z_num), MSK(27)
      double precision xx, yy, zz, MSKS, MSKA, temp1
      double precision a0,a1,a2,a3,a4,a5,a6,a7
      Integer i, j, k, q
      Integer Num_X, Num_Y, Num_Z, NRows
      Integer ii, jj, kk,AA,BB,CC,DD
      character*20 f_name,f_Xcd,f_Ycd,f_Zcd,g_name
      character*50 f_path
      character*70 file_name,file_Xcd,file_Ycd,file_Zcd
      double precision, save :: t_old=-1.d0
      Integer x0, x1, y0, y1, z0, z1
      double precision loc_x,loc_y,loc_z
      integer(8) index_x,index_y,index_z
      integer(8) low_index, high_index, mid_index, iiii
      integer(8) iil,iir,jjf,jjbk,kkb,kkt
      ...
      loc_z=z
	    ...
	    Num_Y=y_num
	    Num_Z=z_num
	    NRows=Num_X*Num_Y*Num_Z    
        
      u=15     
      open (u, File='/hpctmp/e0444165/cp/pdt/T_Inc.dat',status='old')
 
       do 10 i=1,NInc
       read(u, 100) IncN(i), IncT(i)
		    If (t2.GE.IncT(i)) Then
			    IncN1=IncN(i)
			    IncT1=IncT(i)
		    ENDIF
10	   Enddo
100	   format (I5, F24.5)
      ...
      ELSE
		    ...
		    IncT2=IncT(IncN2+1)
      ENDIF
C     Temp(1)=(x-0.0019)*(y+z)*t2
      f_path="/hpctmp/e0444165/cp/pdt/"
      f_name="T000000.dat"
      f_Xcd="Xcd000.dat"
      f_Ycd="Ycd000.dat"
      f_Zcd="Zcd000.dat"
           ...
           ttta=1
		    ...
		    write(f_Xcd(4:6),'(I3.3)') ttta
		    write(file_Xcd,'(A,A)') trim(f_path),f_Xcd
		    
		    write(f_Ycd(4:6),'(I3.3)') ttta
		    write(file_Ycd,'(A,A)') trim(f_path),f_Ycd
		    
		    write(f_Zcd(4:6),'(I3.3)') ttta
		    write(file_Zcd,'(A,A)') trim(f_path),f_Zcd
C		    
		      open (16, File=file_Xcd, status='old')
              ...
              enddo
               ...
               close(16)  
C             
                open (17, File=file_Ycd, status='old')   
              do ii=1,Num_y
                read(17,200) yc1(ii)
              enddo
                ...
                open (18, File=file_Zcd, status='old')
              do ii=1,Num_z
                read(18,200) zc1(ii)
              enddo
      ...
      zc=zc1
        ...
        low_index=iil
        high_index=iir
        do while(high_index-low_index /= 1)
          mid_index=(low_index+high_index)/2
          if (loc_x <= xc(mid_index)) then
            high_index=(low_index+high_index+1)/2
          else !if (loc_x > xc(mid_index)) then
        ...
        do while(high_index-low_index /= 1)
          mid_index=(low_index+high_index)/2
          if (loc_y <= yc(mid_index)) then
            high_index=(low_index+high_index+1)/2
          else !if (loc_y > yc(mid_index)) then
            low_index=(low_index+high_index)/2
          endif
        enddo
        ...
        do while(high_index-low_index /= 1)
          mid_index=(low_index+high_index)/2
          if (loc_z <= zc(mid_index)) then
            high_index=(low_index+high_index+1)/2
          else !if (loc_z > zc(mid_index)) then
            low_index=(low_index+high_index)/2
          endif
        enddo
      ...
      allocate (NTl1(x0:x0,y0:y1,z0:z1)) 
      write(f_name(2:7),'(I6.6)') (IncN1*1000+BB)
 
      write(file_name,'(A,A)') trim(f_path),f_name
      open (19, File=file_name, status='old')     
              do ii=CC,DD
			    do jj=1,Num_Y
			      do kk=1,Num_z
C			    
C        read(19,200) NTl1(ii,jj,kk)
      if ((ii.ge.x0).and.(ii.LE.x0)
     1 .and.(jj.ge.y0).and.(jj.LE.y1)
     2 .and.(kk.ge.z0).and.(kk.LE.z1)) then
          read(19,200) NTl1(ii,jj,kk)
          else
          read(19,200) Nnt2
          endif   
...
C     %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
      ...
      allocate (NTl2(x0:x0,y0:y1,z0:z1))  
      write(f_name(2:7),'(I6.6)') (IncN2*1000+BB)
      write(file_name,'(A,A)') trim(f_path),f_name
      ...
      if ((ii.ge.x0).and.(ii.LE.x1)
     1 .and.(jj.ge.y0).and.(jj.LE.y1)
     2 .and.(kk.ge.z0).and.(kk.LE.z1)) then
          read(20,200) NTl2(ii,jj,kk)
          else
          read(20,200) Nnt2
          endif   
...
C     print *, "BB",BB
  ...
  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!    
      ...
      allocate (NTh1(x1:x1,y0:y1,z0:z1)) 
      write(f_name(2:7),'(I6.6)') (IncN1*1000+BB)
 
      write(file_name,'(A,A)') trim(f_path),f_name
      open (31, File=file_name, status='old')
      
              do ii=CC,DD
			    do jj=1,Num_Y
			      do kk=1,Num_z
			    
       if ((ii.ge.x1).and.(ii.LE.x1)
     1 .and.(jj.ge.y0).and.(jj.LE.y1)
     2 .and.(kk.ge.z0).and.(kk.LE.z1)) then
          read(31,200) NTh1(ii,jj,kk)
          else
          read(31,200) Nnt2
          endif   
...
C     print *, "nth1",nth1        
      ...
      allocate (NTh2(x1:x1,y0:y1,z0:z1))  
      write(f_name(2:7),'(I6.6)') (IncN2*1000+BB)
      write(file_name,'(A,A)') trim(f_path),f_name
      open (20, File=file_name, status='old')  
              do ii=CC,DD
...
200	   format (F16.4)
      allocate (NTAVG(x0:x1,y0:y1,z0:z1))
     
      do k=z0,z1
          do j=y0,y1
		    do i=x0,x1
            
		     IF (IncN1.EQ.IncN2) then
             
             if(i.EQ.x0)then
             NTAvg(i,j,k)=NTl1(i,j,k)
		     ELSE 
             NTAvg(i,j,k)=NTh1(i,j,k)
		     Endif
             
             else 
             
             if(i.EQ.x0)then
      NTAvg(i,j,k)=NTl1(i,j,k)+((NTl2(i,j,k)-NTl1(i,j,k))*(t2-IncT1)/
     &    (IncT2-IncT1))
		     ELSE 
      NTAvg(i,j,k)=NTh1(i,j,k)+((NTh2(i,j,k)-NTh1(i,j,k))*(t2-IncT1)/
     &    (IncT2-IncT1))
		     Endif  
             endif
      ...
      enddo 
        ...
        a0=NTAvg(x0,y0,z0)*xc(x1)*yc(y1)*zc(z1)/
     &   ((xc(x0)-xc(x1))*(yc(y0)-yc(y1))*(zc(z1)-zc(z0)))+
     &    NTAvg(x0,y0,z1)*xc(x1)*yc(y1)*zc(z0)/
     &    ((xc(x0)-xc(x1))*(yc(y0)-yc(y1))*(zc(z0)-zc(z1)))+
     &     NTAvg(x0,y1,z0)*xc(x1)*yc(y0)*zc(z1)/
     &     ((xc(x0)-xc(x1))*(yc(y0)-yc(y1))*(zc(z0)-zc(z1)))+
     &     NTAvg(x0,y1,z1)*xc(x1)*yc(y0)*zc(z0)/
     &      ((xc(x0)-xc(x1))*(yc(y0)-yc(y1))*(zc(z1)-zc(z0)))+
     &     NTAvg(x1,y0,z0)*xc(x0)*yc(y1)*zc(z1)/
     &     ((xc(x0)-xc(x1))*(yc(y0)-yc(y1))*(zc(z0)-zc(z1)))+
     &    NTAvg(x1,y0,z1)*xc(x0)*yc(y1)*zc(z0)/
     &      ((xc(x0)-xc(x1))*(yc(y0)-yc(y1))*(zc(z1)-zc(z0)))+
     &  NTAvg(x1,y1,z0)*xc(x0)*yc(y0)*zc(z1)/
     &      ((xc(x0)-xc(x1))*(yc(y0)-yc(y1))*(zc(z1)-zc(z0)))+
     &   NTAvg(x1,y1,z1)*xc(x0)*yc(y0)*zc(z0)/
     &     ((xc(x0)-xc(x1))*(yc(y0)-yc(y1))*(zc(z0)-zc(z1)))
CC     
      a1=NTAvg(x0,y0,z0)*yc(y1)*zc(z1)
     &  /((xc(x0)-xc(x1))*(yc(y0)-yc(y1))*(zc(z0)-zc(z1)))
     &  +NTAvg(x0,y0,z1)*yc(y1)*zc(z0)
     &   /((xc(x0)-xc(x1))*(yc(y0)-yc(y1))*(zc(z1)-zc(z0)))
     &  +NTAvg(x0,y1,z0)*yc(y0)*zc(z1)
     &   /((xc(x0)-xc(x1))*(yc(y0)-yc(y1))*(zc(z1)-zc(z0)))
     &   +NTAvg(x0,y1,z1)*yc(y0)*zc(z0)
     &   /((xc(x0)-xc(x1))*(yc(y0)-yc(y1))*(zc(z0)-zc(z1)))
     &   +NTAvg(x1,y0,z0)*yc(y1)*zc(z1)
     &  /((xc(x0)-xc(x1))*(yc(y0)-yc(y1))*(zc(z1)-zc(z0)))
     &   +NTAvg(x1,y0,z1)*yc(y1)*zc(z0)
     &  /((xc(x0)-xc(x1))*(yc(y0)-yc(y1))*(zc(z0)-zc(z1)))
     & +NTAvg(x1,y1,z0)*yc(y0)*zc(z1)
     &   /((xc(x0)-xc(x1))*(yc(y0)-yc(y1))*(zc(z0)-zc(z1)))
     &  +NTAvg(x1,y1,z1)*yc(y0)*zc(z0)
     &   /((xc(x0)-xc(x1))*(yc(y0)-yc(y1))*(zc(z1)-zc(z0)))
C     
      a2=NTAvg(x0,y0,z0)*xc(x1)*zc(z1)
     &  /((xc(x0)-xc(x1))*(yc(y0)-yc(y1))*(zc(z0)-zc(z1)))
     &   +NTAvg(x0,y0,z1)*xc(x1)*zc(z0)
     &    /((xc(x0)-xc(x1))*(yc(y0)-yc(y1))*(zc(z1)-zc(z0)))
     &  +NTAvg(x0,y1,z0)*xc(x1)*zc(z1)
     &    /((xc(x0)-xc(x1))*(yc(y0)-yc(y1))*(zc(z1)-zc(z0)))
     &   +NTAvg(x0,y1,z1)*xc(x1)*zc(z0)
     &   /((xc(x0)-xc(x1))*(yc(y0)-yc(y1))*(zc(z0)-zc(z1)))
     &   +NTAvg(x1,y0,z0)*xc(x0)*zc(z1)
     &  /((xc(x0)-xc(x1))*(yc(y0)-yc(y1))*(zc(z1)-zc(z0)))
     &    +NTAvg(x1,y0,z1)*xc(x0)*zc(z0)
     & /((xc(x0)-xc(x1))*(yc(y0)-yc(y1))*(zc(z0)-zc(z1)))
     &  +NTAvg(x1,y1,z0)*xc(x0)*zc(z1)
     &   /((xc(x0)-xc(x1))*(yc(y0)-yc(y1))*(zc(z0)-zc(z1)))
     &   +NTAvg(x1,y1,z1)*xc(x0)*zc(z0)
     &   /((xc(x0)-xc(x1))*(yc(y0)-yc(y1))*(zc(z1)-zc(z0)))
      
      
      a3=NTAvg(x0,y0,z0)*xc(x1)*yc(y1)
     &  /((xc(x0)-xc(x1))*(yc(y0)-yc(y1))*(zc(z0)-zc(z1)))
     &   +NTAvg(x0,y0,z1)*xc(x1)*yc(y1)
     &  /((xc(x0)-xc(x1))*(yc(y0)-yc(y1))*(zc(z1)-zc(z0)))
     &  +NTAvg(x0,y1,z0)*xc(x1)*yc(y0)
     &   /((xc(x0)-xc(x1))*(yc(y0)-yc(y1))*(zc(z1)-zc(z0)))
     &   +NTAvg(x0,y1,z1)*xc(x1)*yc(y0)
     &   /((xc(x0)-xc(x1))*(yc(y0)-yc(y1))*(zc(z0)-zc(z1)))
     &  +NTAvg(x1,y0,z0)*xc(x0)*yc(y1)
     &  /((xc(x0)-xc(x1))*(yc(y0)-yc(y1))*(zc(z1)-zc(z0)))
     &   +NTAvg(x1,y0,z1)*xc(x0)*yc(y1)
     &  /((xc(x0)-xc(x1))*(yc(y0)-yc(y1))*(zc(z0)-zc(z1)))
     &  +NTAvg(x1,y1,z0)*xc(x0)*yc(y0)
     &   /((xc(x0)-xc(x1))*(yc(y0)-yc(y1))*(zc(z0)-zc(z1)))
     &  +NTAvg(x1,y1,z1)*xc(x0)*yc(y0)
     &   /((xc(x0)-xc(x1))*(yc(y0)-yc(y1))*(zc(z1)-zc(z0)))
      
CC     
      a4=NTAvg(x0,y0,z0)*zc(z1)/
     &  ((xc(x0)-xc(x1))*(yc(y0)-yc(y1))*(zc(z1)-zc(z0)))
     &   +NTAvg(x0,y0,z1)*zc(z0)/
     &   ((xc(x0)-xc(x1))*(yc(y0)-yc(y1))*(zc(z0)-zc(z1)))
     &  +NTAvg(x0,y1,z0)*zc(z1)/
     &   ((xc(x0)-xc(x1))*(yc(y0)-yc(y1))*(zc(z0)-zc(z1)))
     &   +NTAvg(x0,y1,z1)*zc(z0)/
     &   ((xc(x0)-xc(x1))*(yc(y0)-yc(y1))*(zc(z1)-zc(z0)))
     &   +NTAvg(x1,y0,z0)*zc(z1)/
     &   ((xc(x0)-xc(x1))*(yc(y0)-yc(y1))*(zc(z0)-zc(z1)))
     &   +NTAvg(x1,y0,z1)*zc(z0)/
     &  ((xc(x0)-xc(x1))*(yc(y0)-yc(y1))*(zc(z1)-zc(z0)))
     &  +NTAvg(x1,y1,z0)*zc(z1)/
     &  ((xc(x0)-xc(x1))*(yc(y0)-yc(y1))*(zc(z1)-zc(z0)))
     &   +NTAvg(x1,y1,z1)*zc(z0)/
     &   ((xc(x0)-xc(x1))*(yc(y0)-yc(y1))*(zc(z0)-zc(z1)))
      
      
      a5=NTAvg(x0,y0,z0)*yc(y1)
     &  /((xc(x0)-xc(x1))*(yc(y0)-yc(y1))*(zc(z1)-zc(z0)))
     &  +NTAvg(x0,y0,z1)*yc(y1)
     &   /((xc(x0)-xc(x1))*(yc(y0)-yc(y1))*(zc(z0)-zc(z1)))
     &  +NTAvg(x0,y1,z0)*yc(y0)
     &   /((xc(x0)-xc(x1))*(yc(y0)-yc(y1))*(zc(z0)-zc(z1)))
     &   +NTAvg(x0,y1,z1)*yc(y0)
     &   /((xc(x0)-xc(x1))*(yc(y0)-yc(y1))*(zc(z1)-zc(z0)))
     &   +NTAvg(x1,y0,z0)*yc(y1)
     &  /((xc(x0)-xc(x1))*(yc(y0)-yc(y1))*(zc(z0)-zc(z1)))
     &   +NTAvg(x1,y0,z1)*yc(y1)
     &  /((xc(x0)-xc(x1))*(yc(y0)-yc(y1))*(zc(z1)-zc(z0)))
     &  +NTAvg(x1,y1,z0)*yc(y0)
     &   /((xc(x0)-xc(x1))*(yc(y0)-yc(y1))*(zc(z1)-zc(z0)))
     &  +NTAvg(x1,y1,z1)*yc(y0)
     &   /((xc(x0)-xc(x1))*(yc(y0)-yc(y1))*(zc(z0)-zc(z1)))
C     
      a6=NTAvg(x0,y0,z0)*xc(x1)
     &  /((xc(x0)-xc(x1))*(yc(y0)-yc(y1))*(zc(z1)-zc(z0)))
     &   +NTAvg(x0,y0,z1)*xc(x1)
     &   /((xc(x0)-xc(x1))*(yc(y0)-yc(y1))*(zc(z0)-zc(z1)))
     & +NTAvg(x0,y1,z0)*xc(x1)
     &   /((xc(x0)-xc(x1))*(yc(y0)-yc(y1))*(zc(z0)-zc(z1)))
     &   +NTAvg(x0,y1,z1)*xc(x1)
     &   /((xc(x0)-xc(x1))*(yc(y0)-yc(y1))*(zc(z1)-zc(z0)))
     &   +NTAvg(x1,y0,z0)*xc(x0)
     &  /((xc(x0)-xc(x1))*(yc(y0)-yc(y1))*(zc(z0)-zc(z1)))
     &  +NTAvg(x1,y0,z1)*xc(x0)
     &  /((xc(x0)-xc(x1))*(yc(y0)-yc(y1))*(zc(z1)-zc(z0)))
     &  +NTAvg(x1,y1,z0)*xc(x0)
     &   /((xc(x0)-xc(x1))*(yc(y0)-yc(y1))*(zc(z1)-zc(z0)))
     & +NTAvg(x1,y1,z1)*xc(x0)
     &   /((xc(x0)-xc(x1))*(yc(y0)-yc(y1))*(zc(z0)-zc(z1)))
C    
       a7=NTAvg(x0,y0,z0)
     &  /((xc(x0)-xc(x1))*(yc(y0)-yc(y1))*(zc(z0)-zc(z1)))
     ...
     &   /((xc(x0)-xc(x1))*(yc(y0)-yc(y1))*(zc(z1)-zc(z0)))
      ...
      temp1=a0+a1*x+a2*y+a3*z+a4*x*y+a5*x*z+a6*y*z+a7*x*y*z
      
      if (temp1.GT.3500) then
      Temp(1)=3500
      else
      Temp(1)=temp1
      endif
      
C     print*, 'temp(1),node',temp(1),node
      deallocate (ntavg)
      deallocate (ntL1,ntL2,ntH1,ntH2)
      RETURN
      END
...
C                          systems GAMERR
      double precision rot_sl(3,3), vec1(3,1),vec2(3,1)
      integer, parameter :: nds =2784             !C5
C     integer, parameter :: nds =21888            !C3
C     integer, parameter :: nds =9288             !C2
C     integer, parameter :: nds =45880            !c1
...
C     
      ...
      call MutexLock( 2 )      ! lock Mutex #1
         ygr(noel)=coords(2)
      call MutexUnlock( 2 )      ! lock Mutex #1
      ...
      statev(NSTATV)=grainID(noel)
C     print*, 'grainID(noel),noel',statev(NSTATV),noel
      
CC     change the grain orientation according to the grain
C       in case that statev(NSTATV)=0, we set +0.1
      ...
      ag1=statev(NSTATV)/10*2+0.3
      ag2=statev(NSTATV)/10-0.2
      ag3=statev(NSTATV)/10*2+0.1
      
      ag1=mod(ag1, 1.5708)
      ag2=mod(ag2, 1.5708)
      ag3=mod(ag3, 1.5708)
      
      rot_sl(1,1)=cos(ag1)*cos(ag2)
      rot_sl(1,2)=cos(ag1)*sin(ag2)*sin(ag3)-sin(ag1)*cos(ag3)
      rot_sl(1,3)=cos(ag1)*sin(ag2)*cos(ag3)+sin(ag1)*sin(ag3)
      rot_sl(2,1)=sin(ag1)*cos(ag2)
      rot_sl(2,2)=sin(ag1)*sin(ag2)*sin(ag3)+cos(ag1)*cos(ag3)
      rot_sl(2,3)=sin(ag1)*sin(ag2)*cos(ag3)-cos(ag1)*sin(ag3)
      rot_sl(3,1)=-sin(ag2)
      rot_sl(3,2)=cos(ag2)*sin(ag3)
      rot_sl(3,3)=cos(ag2)*cos(ag3)
      
      vec1=matmul(rot_sl, vec1)
      
      vec2=matmul(rot_sl, vec2)
...
C
         STATEV(NSTATV1)=FLOAT(NSLPTL)
         DO I=1,NSET
            STATEV(NSTATV1-4+I)=FLOAT(NSLIP(I))
         END DO
...
C
         NSLPTL=NINT(STATEV(NSTATV1))
         DO I=1,NSET
            NSLIP(I)=NINT(STATEV(NSTATV1-4+I))
         END DO
...
CFIXB
      ...
      if (temp.gt.1623.0) then
      
      do i=(9*NSLPTL+1),(10*NSLPTL)
      
      STATEV(i)=0.0001
      
      enddo
      
      STATEV(10*NSLPTL+1)=0.0001
...
C
      ...
      SUBROUTINE getGrainID (statev1,xx,yy,zz,t1)
      
      double precision  t1, t2,statev1
      integer, parameter :: t_stp_num =8
      integer, parameter :: x_num = 51
      integer, parameter :: y_num = 121
      integer, parameter :: z_num = 34
      Integer IncN(t_stp_num), un
      double precision IncT(t_stp_num)
      Integer NInc,Field_out
      Integer IncN1,IncN2,ttta
      double precision IncT1, IncT2
      double precision NNT2
      double precision NTl1
      double precision, save :: xc1(x_num), yc1(y_num), zc1(z_num)
      double precision, save :: xc2(x_num), yc2(y_num), zc2(z_num)
      double precision, save :: xc(x_num), yc(y_num), zc(z_num)
      double precision xx, yy, zz
      Integer ii, j1, k1, q1, NSTATV1
      Integer Num_X, Num_Y, Num_Z, NRows
      Integer iii, jj1, kk1,AA,BB,CC,DD
      character*20 f_name,f_Xcd,f_Ycd,f_Zcd
      character*50 f_path
      character*70 file_name,file_Xcd,file_Ycd,file_Zcd
...
C     print*,"xx,yy,zz",xx,yy,zz
	    ...
	    NRows=Num_X*Num_Y*Num_Z    
          ...
          un=15     
      open (un, File='/hpctmp/e0444165/cp/pdt/T_Inc.dat',status='old')
       do 10 ii=1,NInc
       read(un, 100) IncN(ii), IncT(ii)
		    If (t2.GE.IncT(ii)) Then
			    IncN1=IncN(ii)
			    IncT1=IncT(ii)
		    ENDIF
10	   Enddo
...
C     print *, "x0,y0",x0,y0
      ...
      if ((iii.ge.x0).and.(iii.LE.x0)
     1 .and.(jj1.ge.y0).and.(jj1.LE.y0)
     2 .and.(kk1.ge.z0).and.(kk1.LE.z0)) then
          read(19,*) NTl1
